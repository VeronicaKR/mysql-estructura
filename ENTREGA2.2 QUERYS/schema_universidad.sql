...
/* una query por TIPO alumno y otra por alumno matriculado*/
SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE FROM PERSONA WHERE PERSONA.TIPO='alumno'ORDER BY APELLIDO1,APELLIDO2,NOMBRE ASC;
SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE FROM PERSONA INNER JOIN ALUMNO_SE_MATRICULA_ASIGNATURA ON PERSONA.ID=ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO ORDER BY APELLIDO1,APELLIDO2,NOMBRE ASC;

SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE,TELEFONO FROM PERSONA WHERE PERSONA.TIPO='alumno' AND TELEFONO IS NULL ORDER BY APELLIDO1,APELLIDO2,NOMBRE ASC;
SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE,TELEFONO FROM PERSONA INNER JOIN ALUMNO_SE_MATRICULA_ASIGNATURA ON PERSONA.ID=ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO WHERE TELEFONO IS NULL ORDER BY APELLIDO1,APELLIDO2,NOMBRE ASC ;

SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE,TELEFONO FROM PERSONA WHERE PERSONA.TIPO='alumno' AND FECHA_NACIMIENTO LIKE '%1999%';
SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE,FECHA_NACIMIENTO FROM PERSONA INNER JOIN ALUMNO_SE_MATRICULA_ASIGNATURA ON PERSONA.ID=ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO WHERE PERSONA.FECHA_NACIMIENTO LIKE '%1999%';

SELECT DISTINCT APELLIDO1,APELLIDO2,NOMBRE,TELEFONO,NIF FROM PERSONA WHERE PERSONA.TIPO='profesor' AND TELEFONO IS NULL AND NIF LIKE '%K'ORDER BY APELLIDO1 ASC;
SELECT * FROM ASIGNATURA WHERE ID_GRADO=7 AND CURSO=3 AND CUATRIMESTRE=1;  
SELECT PERSONA.APELLIDO1,PERSONA.APELLIDO2,PERSONA.NOMBRE,DEPARTAMENTO.NOMBRE FROM PROFESOR INNER JOIN PERSONA ON PROFESOR.ID_PROFESOR=PERSONA.ID INNER JOIN DEPARTAMENTO ON PROFESOR.ID_DEPARTAMENTO=DEPARTAMENTO.ID ORDER BY APELLIDO1,APELLIDO2,PERSONA.NOMBRE ;
SELECT ASIGNATURA.NOMBRE,CURSO_ESCOLAR.ANYO_INICIO,CURSO_ESCOLAR.ANYO_FIN FROM ALUMNO_SE_MATRICULA_ASIGNATURA INNER JOIN ASIGNATURA ON ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ASIGNATURA=ASIGNATURA.ID INNER JOIN CURSO_ESCOLAR ON CURSO_ESCOLAR.ID=ALUMNO_SE_MATRICULA_ASIGNATURA.ID_CURSO_ESCOLAR INNER JOIN PERSONA ON ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO=PERSONA.ID WHERE NIF='26902806M';
SELECT DISTINCT DEPARTAMENTO.NOMBRE FROM DEPARTAMENTO INNER JOIN PROFESOR ON DEPARTAMENTO.ID=PROFESOR.ID_DEPARTAMENTO INNER JOIN ASIGNATURA ON PROFESOR.ID_PROFESOR=ASIGNATURA.ID_PROFESOR INNER JOIN GRADO WHERE GRADO.NOMBRE ='Grado en Ingeniería Informática (Plan 2015)';
SELECT DISTINCT  PERSONA.NOMBRE,ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO,CURSO_ESCOLAR.ANYO_INICIO,CURSO_ESCOLAR.ANYO_FIN FROM ALUMNO_SE_MATRICULA_ASIGNATURA INNER JOIN PERSONA ON ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO=PERSONA.ID INNER JOIN CURSO_ESCOLAR ON ALUMNO_SE_MATRICULA_ASIGNATURA.ID_CURSO_ESCOLAR=CURSO_ESCOLAR.ID WHERE ANYO_INICIO='2018'AND ANYO_FIN='2019'ORDER BY PERSONA.NOMBRE ASC;



/*LEFT-RIGHT*/
SELECT DEPARTAMENTO.NOMBRE,PERSONA.APELLIDO1,PERSONA.APELLIDO2,PERSONA.NOMBRE FROM PROFESOR RIGHT JOIN DEPARTAMENTO ON DEPARTAMENTO.ID=PROFESOR.ID_DEPARTAMENTO RIGHT JOIN PERSONA ON PERSONA.ID=PROFESOR.ID_PROFESOR WHERE PERSONA.TIPO='Profesor' ORDER BY DEPARTAMENTO.NOMBRE,PERSONA.APELLIDO1,PERSONA.APELLIDO2,PERSONA.NOMBRE DESC;
SELECT PERSONA.APELLIDO1,PERSONA.APELLIDO2,PERSONA.NOMBRE,DEPARTAMENTO.NOMBRE FROM PERSONA LEFT JOIN PROFESOR ON PROFESOR.ID_PROFESOR=PERSONA.ID RIGHT JOIN DEPARTAMENTO ON DEPARTAMENTO.ID=PROFESOR.ID_PROFESOR WHERE DEPARTAMENTO.NOMBRE IS NULL;
SELECT DEPARTAMENTO.NOMBRE,PERSONA.APELLIDO1,PERSONA.APELLIDO2,PERSONA.NOMBRE FROM PROFESOR RIGHT JOIN DEPARTAMENTO ON DEPARTAMENTO.ID=PROFESOR.ID_DEPARTAMENTO LEFT JOIN PERSONA ON PERSONA.ID=PROFESOR.ID_PROFESOR WHERE APELLIDO1 IS NULL;
SELECT DISTINCT PROFESOR.ID_PROFESOR,PERSONA.NOMBRE,PERSONA.APELLIDO1,PERSONA.APELLIDO2 FROM PROFESOR LEFT JOIN PERSONA ON PROFESOR.ID_PROFESOR=PERSONA.ID LEFT JOIN ASIGNATURA ON ASIGNATURA.ID_PROFESOR IS NULL ORDER BY PROFESOR.ID_PROFESOR ASC;
SELECT DISTINCT ASIGNATURA.NOMBRE,ASIGNATURA.ID_PROFESOR FROM ASIGNATURA RIGHT JOIN PROFESOR ON ASIGNATURA.ID_PROFESOR IS NULL;


/*RESUM*/
SELECT * FROM PERSONA WHERE TIPO='alumno';
SELECT COUNT(*) FROM PERSONA WHERE TIPO='alumno' AND FECHA_NACIMIENTO LIKE '%1999%';
SELECT DEPARTAMENTO.NOMBRE, COUNT(*) AS NUMERO_PROFESORES FROM DEPARTAMENTO INNER JOIN PROFESOR ON DEPARTAMENTO.ID=PROFESOR.ID_DEPARTAMENTO GROUP BY DEPARTAMENTO.NOMBRE HAVING COUNT(*) ORDER BY NUMERO_PROFESORES ASC ;
SELECT DEPARTAMENTO.NOMBRE,COUNT(ID_PROFESOR) AS NUMERO_DE_PROFESORES FROM DEPARTAMENTO LEFT JOIN PROFESOR ON DEPARTAMENTO.ID=PROFESOR.ID_DEPARTAMENTO GROUP BY DEPARTAMENTO.NOMBRE HAVING COUNT(*) ORDER BY NUMERO_DE_PROFESORES ASC;
SELECT GRADO.NOMBRE,COUNT(ASIGNATURA.ID) AS NUMERO_ASIGNATURAS FROM GRADO LEFT JOIN ASIGNATURA ON GRADO.ID=ASIGNATURA.ID_GRADO OR ASIGNATURA.ID_GRADO IS NULL GROUP BY GRADO.NOMBRE;
SELECT GRADO.NOMBRE,COUNT(*) AS NUMERO_ASIGNATURAS FROM GRADO LEFT JOIN ASIGNATURA ON GRADO.ID=ASIGNATURA.ID_GRADO GROUP BY GRADO.NOMBRE HAVING COUNT (ASIGNATURA.ID) > 40;
SELECT GRADO.NOMBRE,ASIGNATURA.TIPO, SUM(ASIGNATURA.CREDITOS) AS TOTAL_CREDITOS FROM GRADO INNER JOIN ASIGNATURA ON GRADO.ID=ASIGNATURA.ID_GRADO GROUP BY GRADO.NOMBRE,ASIGNATURA.TIPO HAVING SUM(ASIGNATURA.TIPO);
SELECT CURSO_ESCOLAR.ANYO_INICIO, COUNT(ALUMNO_SE_MATRICULA_ASIGNATURA.ID_CURSO_ESCOLAR) AS TOTAL_MATRICULADOS FROM ALUMNO_SE_MATRICULA_ASIGNATURA  INNER JOIN CURSO_ESCOLAR ON ALUMNO_SE_MATRICULA_ASIGNATURA.ID_CURSO_ESCOLAR= CURSO_ESCOLAR.ID GROUP BY CURSO_ESCOLAR.ANYO_INICIO HAVING COUNT(ALUMNO_SE_MATRICULA_ASIGNATURA.ID_ALUMNO);
SELECT PROFESOR.ID_PROFESOR,PERSONA.NOMBRE,PERSONA.APELLIDO1,PERSONA.APELLIDO2, COUNT(ASIGNATURA.NOMBRE) AS NUMERO_ASIGNATURAS FROM PROFESOR INNER JOIN PERSONA ON PROFESOR.ID_PROFESOR=PERSONA.ID LEFT JOIN ASIGNATURA ON PROFESOR.ID_PROFESOR=ASIGNATURA.ID_PROFESOR GROUP BY PROFESOR.ID_PROFESOR HAVING COUNT (PROFESOR.ID_PROFESOR)ORDER BY NUMERO_ASIGNATURAS DESC;
SELECT *, MIN(FECHA_NACIMIENTO) FROM PERSONA WHERE TIPO='alumno';
SELECT DISTINCT * FROM PROFESOR INNER JOIN ASIGNATURA WHERE ASIGNATURA.ID_PROFESOR IS NULL GROUP BY PROFESOR.ID_PROFESOR ORDER BY PROFESOR.ID_PROFESOR ASC ;
